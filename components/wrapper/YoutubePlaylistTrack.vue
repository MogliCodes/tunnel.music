...
const videoUrl = ref()
const { data, error } = await useFetch(`https://www.googleapis.com/youtube/v3/videos?id=${videoId}&part=player&key=${apiKey}`)

videoUrl.value = data?.value?.items?.[0].player.embedHtml.match(/src="([^"]+)"/)[1];
